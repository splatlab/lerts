																...
																flag_spin)
{
	...
	if (!flag_spin) {
		...
		cf->wait_times[idx].locks_acquired_single_attempt++;
		cf->wait_times[idx].total_time_single += BILLION * (end.tv_sec -
																												start.tv_sec) +
		...
		if (!__sync_lock_test_and_set(lock, 1)) {
			...
			while (__sync_lock_test_and_set(lock, 1))
				while (*lock);
			clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);
			cf->wait_times[idx].total_time_spinning += BILLION * (end.tv_sec -
																														start.tv_sec) +
				end.tv_nsec - start.tv_nsec;
		}
		ret = true;
	}
	cf->wait_times[idx].locks_taken++;
	...
	/*if (!__sync_lock_test_and_set(lock, 1)) {*/
		/*clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);*/
		/*cf->wait_times[idx].locks_acquired_single_attempt++;*/
		/*cf->wait_times[idx].total_time_single += BILLION * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;*/
	/*} else {*/
		/*while (__sync_lock_test_and_set(lock, 1))*/
			/*while (*lock);*/
		/*clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);*/
		/*cf->wait_times[idx].total_time_spinning += BILLION * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;*/
	/*}*/
...
{
#ifdef LOG_WAIT_TIME
	qf_spin_lock(cf, &cf->metadata_lock,cf->num_locks, true);
#else
	qf_spin_lock(&cf->metadata_lock, true);
#endif
	*metadata = *metadata + cnt;
	qf_spin_unlock(&cf->metadata_lock);
	return;
}
...
static inline void set_slot(const QF *qf, uint64_t index, uint64_t value)
{
	...
	get_block(qf, index / SLOTS_PER_BLOCK)->slots[index % SLOTS_PER_BLOCK] =
		value & BITMASK(qf->bits_per_slot);
}
...
static inline uint64_t get_slot(const QF *qf, uint64_t index)
{
	...
	uint64_t *p = (uint64_t *)&get_block(qf, index /
																			 SLOTS_PER_BLOCK)->slots[(index %
																																SLOTS_PER_BLOCK)
																			 * qf->bits_per_slot / 8];
	return (uint64_t)(((*p) >> (((index % SLOTS_PER_BLOCK) * qf->bits_per_slot)
															% 8)) & BITMASK(qf->bits_per_slot));
}
...
#if BITS_PER_SLOT == 8 || BITS_PER_SLOT == 16 || BITS_PER_SLOT == 32 || BITS_PER_SLOT == 64
		fwrite(qf->blocks, sizeof(qfblock), qf->nblocks, fout);
#else
		fwrite(qf->blocks, sizeof(qfblock) + SLOTS_PER_BLOCK * qf->bits_per_slot /
					 8, qf->nblocks, fout);
#endif
...
void qf_deserialize(QF *qf, const char *filename)
{
	...
	qf->num_locks = (qf->xnslots/NUM_SLOTS_TO_LOCK)+2;
	qf->metadata_lock = 0;
	/* initialize all the locks to 0 */
	qf->locks = (volatile int *)calloc(qf->num_locks, sizeof(volatile int));
...
#else
	qf->blocks = (qfblock *)calloc(qf->nblocks, sizeof(qfblock) +
																 SLOTS_PER_BLOCK * qf->bits_per_slot / 8);
#endif

#if BITS_PER_SLOT == 8 || BITS_PER_SLOT == 16 || BITS_PER_SLOT == 32 || BITS_PER_SLOT == 64
		fread(qf->blocks, sizeof(qfblock), qf->nblocks, fin);
#else
		fread(qf->blocks, sizeof(qfblock) + SLOTS_PER_BLOCK * qf->bits_per_slot /
					8, qf->nblocks, fin);
#endif
																																										 ...
																																										 uint64_t		 noverwrites)
{
	...
	/*modify_metadata(qf, &qf->noccupied_slots, ninserts);*/
}
																																										 ...
																																										 uint64_t		 nremovals)
{
	...
	/*modify_metadata(qf, &qf->noccupied_slots, -nremovals);*/
}
...
static inline bool insert1(QF *qf, __uint128_t hash, bool lock, bool spin)
{
	...
	if (lock) {
#ifdef LOG_WAIT_TIME
		if (!qf_spin_lock(qf, &qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK],
											hash_bucket_index/NUM_SLOTS_TO_LOCK, spin))
			return false;
		if (hash_bucket_lock_offset > (NUM_SLOTS_TO_LOCK - CLUSTER_SIZE)) {
			if (!qf_spin_lock(qf, &qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK+1],
												hash_bucket_index/NUM_SLOTS_TO_LOCK+1, spin)) {
				qf_spin_unlock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK]);
				return false;
			}
		}
#else
		if (!qf_spin_lock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK], spin))
			return false;
		if (hash_bucket_lock_offset > (NUM_SLOTS_TO_LOCK - CLUSTER_SIZE)) {
			if (!qf_spin_lock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK+1],
												spin)) {
				qf_spin_unlock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK]);
				return false;
			}
	...
	if (is_empty(qf, hash_bucket_index) /* might_be_empty(qf, hash_bucket_index) && runend_index == hash_bucket_index */) {
		...
		/*modify_metadata(qf, &qf->ndistinct_elts, 1);*/
		/*modify_metadata(qf, &qf->noccupied_slots, 1);*/
		/*modify_metadata(qf, &qf->nelts, 1);*/
	} else {
	...
	if (lock) {
		qf_spin_unlock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK]);
		if (hash_bucket_lock_offset > (NUM_SLOTS_TO_LOCK - CLUSTER_SIZE))
			qf_spin_unlock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK+1]);
	}
													...
													bool spin)
{
	...
	if (lock) {
		...
		if (!qf_spin_lock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK], spin))
			return false;
		if (hash_bucket_lock_offset > (NUM_SLOTS_TO_LOCK - CLUSTER_SIZE)) {
			if (!qf_spin_lock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK+1], spin)) {
				qf_spin_unlock(&qf->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK]);
				return false;
			}
		}
	...
	if (might_be_empty(qf, hash_bucket_index) && runend_index ==
		...
		/*modify_metadata(qf, &qf->nelts, count);*/
	}
...
void qf_init(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t value_bits)
{
	...
	qf->locks = (volatile int *)calloc(qf->num_locks, sizeof(volatile int));
#ifdef LOG_WAIT_TIME
	qf->wait_times = (wait_time_data* )calloc(qf->num_locks+1,
...
#ifdef LOG_WAIT_TIME
	memset(qf->wait_times, 0, (qf->num_locks+1)*sizeof(wait_time_data));
#endif
#if BITS_PER_SLOT == 8 || BITS_PER_SLOT == 16 || BITS_PER_SLOT == 32 || BITS_PER_SLOT == 64
	memset(qf->blocks, 0, qf->nblocks* sizeof(qfblock));
#else
	memset(qf->blocks, 0, qf->nblocks*(sizeof(qfblock) + SLOTS_PER_BLOCK *
																		 qf->bits_per_slot / 8));
#endif
}
...
void qf_iterator(QF *qf, QFi *qfi, uint64_t position)
{
	...
	qfi->c_info = (cluster_data* )calloc(qf->nslots/32, sizeof(cluster_data));
...
{
	assert(qfi->current < qfi->qf->nslots);
	...
	*count = current_count; 
	...
	qfi->qf->ndistinct_elts++;
	qfi->qf->nelts += current_count;
