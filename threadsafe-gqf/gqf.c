...
static void modify_metadata(QF *qf, uint64_t *metadata, int cnt, bool lock)
{
	if (lock) {
#ifdef LOG_WAIT_TIME
																																										 ...
																																										 bool lock)
{
	...
	modify_metadata(qf, &qf->metadata->noccupied_slots, ninserts, lock);
}
...
static inline bool insert1(QF *qf, __uint128_t hash, bool lock, bool spin)
{
	...
	if (is_empty(qf, hash_bucket_index) /* might_be_empty(qf, hash_bucket_index) && runend_index == hash_bucket_index */) {
		...
		modify_metadata(qf, &qf->metadata->ndistinct_elts, 1, lock);
		modify_metadata(qf, &qf->metadata->noccupied_slots, 1, lock);
		modify_metadata(qf, &qf->metadata->nelts, 1, lock);
	} else {
													...
													bool spin)
{
	...
	if (might_be_empty(qf, hash_bucket_index) && runend_index ==
		...
		if (!is_occupied(qf, hash_bucket_index)) { /* Empty bucket, but its slot is occupied. */
			...
			if (current_remainder < hash_remainder) {
				...
				insert_replace_slots_and_shift_remainders_and_runends_and_offsets(qf, 
																																					...
																																					p, 
																																					&new_values[67] - p, 
																																					current_end - runstart_index + 1, lock);
		...
		modify_metadata(qf, &qf->metadata->nelts, count, lock);
	}
...
void qf_merge(QF *qfa, QF *qfb, QF *qfc, bool lock, bool spin) 
{
	...
	do {
		if (keya < keyb) {
			qf_insert(qfc, keya, valuea, counta, lock, spin);
			qfi_next(&qfia);
			qfi_get(&qfia, &keya, &valuea, &counta);
		}
		else {
			qf_insert(qfc, keyb, valueb, countb, lock, spin);
			qfi_next(&qfib);
			qfi_get(&qfib, &keyb, &valueb, &countb);
		}
